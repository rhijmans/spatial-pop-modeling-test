...
start <- Sys.time()
 for (t in 1:20){
  print(t)
   print(Sys.time() - start); flush.console()
...
#    pol_ls_score[,,s] <- terra::as.matrix(HN * forage, wide = T)
    
    #Disperse queens
    if (t > 1) {
      max_queen_array[,,s,t] <- max_nest * HN #Calculate maximum number of queens per cell given landscape conditions
      ...
      disperse[,,s,t] <- terra::as.matrix(terra::focal(rep_rast, filter), wide = T) #convolution of dispersal of new queens 
      ...
      rep_rast <- terra::rast(rep)
      disperse[,,s,t] <- terra::as.matrix(terra::focal(rep_rast, filter), wide = T)
    }
